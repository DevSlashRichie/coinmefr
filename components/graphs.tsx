      ...
      <div className="bg-[#F8FAF5] rounded-xl p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">
            ...
            {selectedMetric === 'rendimiento' && 'Rendimiento en cuenta de inversión'}
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart
              data={
                selectedMetric === 'flujo'
                  ? metrics.flujo
                  : metrics[selectedMetric].map((item: { date: string; value: number }) => ({
                      date: item.date,
                      value: item.value,
                    }))
              }
            ...
            >
              ...
              {selectedMetric === 'flujo' ? (
                <>
                  <Line type="monotone" dataKey="ingresos" stroke="#A7E96B" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="salidas" stroke="#FF6B6B" strokeWidth={2} dot={false} />
                </>
              ) : (
                <Line type="monotone" dataKey="value" stroke="#A7E96B" strokeWidth={2} dot={false} />
              )}
            </LineChart>
          </ResponsiveContainer>
        </div>
